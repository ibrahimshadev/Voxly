...
async fn test_stop_and_process_success() {
    ...
    let mut updates = vec![];
    let result = manager
        .stop_and_process(|update| updates.push(update))
        .await;
...
async fn test_stop_and_process_emits_formatting_when_mode_active() {
    ...
    assert_eq!(updates[3].text, Some("Formatted: Hello world".to_string()));
}
...
async fn test_stop_and_process_falls_back_when_formatting_fails() {
    ...
    assert!(updates
        .iter()
        .any(|update| update.state == DictationState::Formatting));
    assert_eq!(
        updates.last().and_then(|update| update.text.clone()),
